...
async def process_data(foot_name, last_timestamp, websocket):
    ...
    if latest_timestamp != last_timestamp:
        ...
        # Extract force/pressure array
        force = np.array([item.get("force", 0) for item in sorted_data])
        ...
        force_dcb = force - np.mean(force)  # DC bias removal for force
        ...
        force_filt = medfilt(force_dcb, kernel_size=KERNEL_SIZE_FORCE)  # Median filtering for force
